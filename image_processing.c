...
void pictureRedder (Picture * pic)
{
  ...
  //char redVal = ((*pic).pix_array).R;
  //char greVal = ((*pic).pix_array).G;
  //char bluVal = ((*pic).pix_array).B;
  int row, col;
      ...
      for (col = 0; col < (*pic).width; col++)
        {
          if (((((*pic).pix_array[row][col]).R) > 0) && ((((*pic).pix_array[row][col]).R) <= 130))
            {
              Pixel redder = { ((((*pic).pix_array[row][col]).R) += 200), ((*pic).pix_array[row][col]).G, ((*pic).pix_array[row][col]).B};
